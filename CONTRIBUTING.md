...
Line coverage at 100% does not mean tested. Branch coverage at 100% on your diff means the conditional has been thought about - but thought-about is not *observed*. A diff at 100% branch coverage can still be a field of paper tigers: every line runs and nothing asserts the outcome. A test earns its place only when you can name a wrong implementation it reddens against - and for a two-way decision, when you assert *both* directions, not only the refusal. The full doctrine (observe don't just execute, the paper-tiger taxonomy, and mutation testing as the periodic audit that measures it) lives in the `cybersquad-tests` skill; mutation-audit findings and the diff-scoped CI plan are tracked on #232.
